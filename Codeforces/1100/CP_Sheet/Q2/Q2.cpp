...
void solve() {
 ...
 FOR(i,0,n) cin>>bc[i];
 ll ans=0;
 ll a_tot=accumulate(all(ac),0);
 ll b_max=*max_element(all(bc));
 ll xy=0;
 ll total=-1;
 if((k-n)>=0)
 {
 ...
 if(total==-1)
 {
    total=accumulate(ac.begin(),ac.begin()+k,0);
 }
 ll maxi=total;
 ll st=0,maxi_b=bc[0],k_sum=ac[0];
 for(int i=1;i<n && i<k;i++)
 {
    
    ll sr=k_sum+(k-i)*maxi_b;
    maxi=max(maxi,sr);
    k_sum+=ac[i];
    maxi_b=max(maxi_b,bc[i]);
 }
