...
bool isSorted(const vi &a) { int n = len(a); FOR(i, 1, n) { if (a[i - 1] > a[i]) return false;} return true;}
...
Problem Name: B. Counting Orders
Problem Link: https://codeforces.com/problemset/problem/1827/A
Approach : Combinatrics and Sorting

Sort the first array ascending order
Sort the second arrray descending order
using binary search find no. of element in a[] which in more than b[i] store it in mt

remove the greater element of a[i]  that is (mt-i) this will remove earlier element who already considered their presence 
store it in res=res*(mt-i)
res=res*max(mt-i,0ll)%MOD; this is to handle edge cases when no element is found
...
void solve() {
   ...
   for(ll i=0;i<n;i++)
   {
        ll pt=n-(upper_bound(all(a),b[i])-a.begin());
        res=(res*max(pt-i,0ll))%MOD;
   }
   cout<<res<<endl;
