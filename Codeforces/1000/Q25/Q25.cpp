...
/*
Problem Name: B. B. Valerii Against Everyone
Problem Link: https://codeforces.com/problemset/problem/1438/B
Approach : Main idea is to check if there are any duplicate elements in the array or not.
          If there are duplicate elements then print "YES" else print "NO".
          To check duplicate elements, we can use set data structure.
          If the size of the set is equal to the size of the array then there are no duplicate elements.
          Else there are duplicate elements. This will work because 2's power logic will work;
          If all no. are unique the no two sum will have same value of any subarray.
*/
void solve() {
  int n;
  cin>>n;
