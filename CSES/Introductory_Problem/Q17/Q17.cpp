...
Approach : Recursion
*/
void help(vector<string>& board,int row, vector<bool>& cr, vector<bool>& lrd, vector<bool>& rtd, int &ans)
{
    if(row==8)
    {
        ans+=1;
    ...
    for(int i=0;i<8;i++)
    {
        if(board[row][i] =='*' || lrd[row-i+8] || rtd[row+i] ||cr[i] )continue;
        cr[i]=lrd[row-i+8]=rtd[row+i]=true;
        help(board,row+1,cr,lrd,rtd,ans);
        cr[i]=lrd[row-i+8]=rtd[row+i]=false;
...
void solve() {
 ...
 vector<bool> cr(8,false),ltd(8,false),rtd(8,false);
 int ans=0;
 help(board,0,cr,ltd,rtd,ans);
 cout<<ans;
